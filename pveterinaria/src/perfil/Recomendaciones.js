...
const Recomendaciones = () => {
  ...
  const obtenerCitas = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        const servicios = [...new Set(userData1.map(cita => cita.NombreServicio))];
  ...
  useEffect(() => {
    obtenerCitas();
  }, [obtenerCitas]); // `obtenerCitas` es ahora estable y se puede usar como dependencia
  ...
  useEffect(() => {
    if (Array.isArray(serviciosSeleccionados) && Array.isArray(reglas)) {
      const nuevasRecomendaciones = reglas
        .filter(regla => {
          return (
            Array.isArray(regla.antecedents) &&
            regla.antecedents.every(servicio =>
          ...
          {recomendaciones.length > 0 ? (
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
              {recomendaciones.map((recomendacion, index) => (
                ...
                >
                  <img
                    src="https://img.freepik.com/vector-gratis/ilustracion-icono-vector-dibujos-animados-hueso-mordedura-perro-pug-lindo-naturaleza-animal-icono-concepto-aislado-premium_138676-7370.jpg?size=338&ext=jpg"
                    alt="Imagen de recomendación"
