...
export default function Login() {
      ...
      try {
        ...
        if (result === 'Error en tus credenciales') {
          ...
          Swal.fire({
            ...
            text: 'Ahora ve y navega en nuestra página.',
          });
  ...
  const onSuccess = async (response) => {
    ...
    try {
      const result = await fetch(apiurll + "api/CasaDelMarisco/VerificarCorreo", {
        method: "POST",
        body: data,
      }).then((res) => res.json());
      ...
      if (result === "Correo Existe") {
        const userData = await obtenerDatosUsuario(email);
        loginUser(userData);
        navigate(userData.Rol === 2 ? "/Pedidos_admin" : "/");
      }
    } catch (error) {
    ...
    <Layout>
      <div className="registro-form-containerLogin">
        <div className="registro-image-containerLogin w-full lg:w-1/2 flex justify-center lg:justify-end">
          <img src={imagen} alt="Registro" className="registro-imageLogin w-full max-w-xs lg:max-w-md h-auto" />
        </div>
        ...
        <div className="registro-formLogin w-full lg:w-1/2">
          ...
          <form onSubmit={handleSubmit}>
            <label htmlFor="email" className="loginLabel">Correo electrónico :</label>
            <input
              type="email"
              id="email"
              name="email"
            ...
            <div className="password-input-container">
              <input
                ...
                required
              />
              <button type="button" onClick={togglePasswordVisibility} className="btn-visibility">
            ...
            <button className="btn btn-warning" type="submit" disabled={isButtonDisabled}>
              Entrar
            </button>
          </form>

          <div className="container">
            <Link to="/recuperar" className="singText">¿Olvidaste tu password?</Link>
            <Link to="/registro" className="singText ms-3">¿Sin cuenta? Regístrate</Link>
          </div>
        </div>
      </div>
    </Layout>
